...
namespace WebSearcher {
    public class UniversityOfBirmingham : Searcher {
        ...
        public override void GetRes(object o) {
            ...
            for (int i = start; i < end; i++) {
                ...
                try {
                    ...
                    string uri = "http://findit.bham.ac.uk/primo_library/libweb/action/dlSearch.do?institution=44BIR&vid=44BIR_VU1&search_scope=LSCOP_44BIR_RES_PUB&onCampus=false&indx=1&bulkSize=10&dym=true&highlight=true&displayField=title&shown_query=Comparison+of+imputation+methods+for+handling+missing+covariate+data+when+fitting+a+Cox+proportional+hazards+model%3A+a+resampling+study&query=any%2Ccontains%2C"+title+"&pure_sub=SEARCH";
        ...
        private string DealStr(string str_o) {
            string str = str_o;
            str = str.Replace("<span class=\"searchword\">", "").Replace("</span>", "").Replace(".","");
