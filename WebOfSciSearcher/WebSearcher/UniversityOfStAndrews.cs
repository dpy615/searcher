...
namespace WebSearcher {
    public class UniversityOfStAndrews : Searcher {
        ...
        public override void GetRes(object o) {
            ...
            for (int i = start; i < end; i++) {
                ...
                try {
                    ...
                    string title = title_old.Replace("&","%26");

                    string uri = "https://research-repository.st-andrews.ac.uk/simple-search?query="+title;
                    ...
                    res = DealStr(res);
                    if (res.ToLower().Contains((title_old.Replace(" ", "") + "</a>").ToLower())) {
                        dt.Rows[i]["isIn"] = "YES";
                        yes++;
                    } else {
                        dt.Rows[i]["isIn"] = "NO";
