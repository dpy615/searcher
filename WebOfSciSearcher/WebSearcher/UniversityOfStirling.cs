...
namespace WebSearcher {
    public class UniversityOfStirling : Searcher {
        ...
        public override void GetRes(object o) {
            ...
            for (int i = start; i < end; i++) {
                ...
                try {
                    ...
                    string uri = "https://dspace.stir.ac.uk/simple-search?location=%2F&query="+title+"&rpp=10&sort_by=score&order=desc";
                    ...
                    res = DealStr(res);
                    string ttt = (title_old.Replace("-", "&#x20;").Replace("&", "&amp;").Replace(" ", "&#x20;") + "</a>").ToLower();
                    if (res.ToLower().Contains((title_old.Replace("&", "&amp;").Replace("-", "&#x20;").Replace(" ", "&#x20;") + "</a>").ToLower())) {
                        dt.Rows[i]["isIn"] = "YES";
                        yes++;
                    } else {
                        dt.Rows[i]["isIn"] = "NO";
